...
migrationStorage.on('migrateElement', (elementData) => {
  if (elementData.tag === 'vc_raw_html') {
    let baseAttrs = elementData._generalElementAttributes
    baseAttrs.rawHtml = decodeURIComponent(utils.base64decode(elementData._subInnerContent.replace(/^#E-8_/, '').trim()))
    baseAttrs.rawHtml = baseAttrs.rawHtml.replace(/<\/?script[^>]*>/g, '')
    const attrs = Object.assign({}, baseAttrs, { tag: 'rawHtml' })
