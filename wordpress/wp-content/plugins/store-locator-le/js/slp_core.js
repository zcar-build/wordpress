    ...
    slp = slp || {
		...
		geocoder: {
			'geocode': function(request, callback) {
				jQuery.getJSON(encodeURI(slplus.rest_url + 'geocode/' + slplus.apikey + '/' + request.address)).done(function(resp) {
					callback(resp.results, resp.status);
				}).fail(function(resp) {
...
slp.setup_map = function () {
	...
	if ( ( ! slplus.options.use_sensor ) ||  ( slplus.options.use_sensor === '0' ) ) {
		...
		if ( slplus.options.map_center_lat && slplus.options.map_center_lng ) {
			var map_center = new google.maps.LatLng( slplus.options.map_center_lat , slplus.options.map_center_lng );
...
var slp_Map = function () {
	 ...
	 * @param status
	 */
	this.process_geocode_response = function (results , status ) {
		if ( ( status === google.maps.GeocoderStatus.OK ) && ( results.length > 0 ) ) {
			...
			//
				...
				console.log( 'new home point: ' + _this.homePoint.lat() + ' , ' + _this.homePoint.lng() );

				//do a search based on settings
				var radius = _this.saneValue( 'radiusSelect' , this.default_radius );
				_this.load_markers( geocode_results.best.geometry.location , radius );
			}
			...
			if ( _this.gmap === null ) {
				...
				_this.clearMarkers();
				slp.log( 'Google JavaScript API geocoder failed with status ' + status + ' results ' + results.length );
			}
		}
	};
